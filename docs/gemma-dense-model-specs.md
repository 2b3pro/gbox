...
**Relevance to gbox:** These are the dense bundles gbox runs on-device. `E2B` is the default (`gbox --default` → `gemma-4-E2B-it`), `E4B` is the high-cognition model (`gbox --high` → `gemma-4-E4B-it`), and `12B Unified` corresponds to the `gemma4-12b` bundle discoverable in the LiteRT-LM registry. Use `gbox modelinfo <model>` to confirm a local bundle's modalities and MTP draft head.
